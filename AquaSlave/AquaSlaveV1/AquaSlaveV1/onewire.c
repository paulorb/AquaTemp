...
/*******************************************/
// the bus is on PD6:
//#define OW_GET_IN()    (PINC&(1<<PINC1))
//// output and low:
//#define OW_DIR_OUT_LOW()   PORTC&=~(1<<PORTC1);DDRC|=1<<DDC1
//// output and high:
//#define OW_DIR_OUT_HIGH()   PORTC|=(1<<PORTC1);DDRC|=1<<DDC1
//// if input the also pullup on:
//#define OW_DIR_IN()    DDRC&=~(1<<DDC1);PORTC|=1<<PORTC1

#define OW_GET_IN()    (PIND&(1<<PIND5))
// output and low:
#define OW_DIR_OUT_LOW()   PORTD&=~(1<<PORTD5);DDRD|=1<<DDRD5
// output and high:
#define OW_DIR_OUT_HIGH()   PORTD|=(1<<PORTD5);DDRD|=1<<DDRD5
// if input the also pullup on:
#define OW_DIR_IN()    DDRD&=~(1<<DDRD5);PORTD|=1<<PORTD5
